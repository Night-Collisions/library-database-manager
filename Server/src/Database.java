...
class Database {
    ...
    String getAuthorsNotOfPubl(String p_id) {
        String query =
                ...
                "WHERE NOT EXISTS(SELECT 1 " +
                                 ...
                                 "WHERE ap.authors_id = a.authors_id " +
                                   "AND ap.publications_id = ?)";
        return execPSWithId(p_id, query);
    }
    ...
    String getEditorsNotOfPubl(String p_id) {
        String query =
                ...
                "FROM editors e " +
                "WHERE NOT EXISTS(SELECT 1 " +
                                 "FROM editors_publications ep " +
                                 "WHERE ep.editors_id = e.editors_id " +
                                   "AND ep.publications_id = ?)";
        return execPSWithId(p_id, query);
    }
    ...
    String getKeywordsNotOfPubl(String id) {
        String s =
                ...
                "FROM keywords k " +
                "WHERE NOT EXISTS(SELECT 1 " +
                                 "FROM publications_keywords pk " +
                                 "WHERE pk.keywords_id = k.keywords_id " +
                                   "AND pk.publications_id = ?)";
        return execPSWithId(id, s);
    }
    ...
    String getUdcNotOfPubl(String id) {
        String s =
                ...
                "FROM udc_codes uc " +
                "WHERE NOT EXISTS(SELECT 1 " +
                                 "FROM publications_udc_codes puc " +
                                 "WHERE puc.udc_codes_id = uc.udc_codes_id " +
                                   "AND puc.publications_id = ?)";
        return execPSWithId(id, s);
    }
